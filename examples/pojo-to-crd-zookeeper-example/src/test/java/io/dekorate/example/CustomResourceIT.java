...
class CustomResourceIT implements WithProject {
  ...
  void shouldApplyZookeeper() {
    ...
    assertTrue(Exec.inProject(getProject()).commands("oc", "create", "-f", testResources.resolve("test-zookeeper.yml").toAbsolutePath().toString()));
    assertTrue(Exec.inProject(getProject()).commands("oc", "delete", "-f", testResources.resolve("test-zookeeper.yml").toAbsolutePath().toString()));
