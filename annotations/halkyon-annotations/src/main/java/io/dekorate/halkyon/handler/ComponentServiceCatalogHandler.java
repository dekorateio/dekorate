...
public class ComponentServiceCatalogHandler implements Handler<ServiceCatalogConfig> {
  ...
  private io.dekorate.halkyon.model.Parameter[] convertToModelParameter(Parameter[] parametersConfig) {
    io.dekorate.halkyon.model.Parameter[] parameters = new io.dekorate.halkyon.model.Parameter[parametersConfig.length];
    io.dekorate.halkyon.model.Parameter parameter;
    
    for (int i = 0; i < parametersConfig.length; i++) {
      Parameter paramConfig = parametersConfig[i];
      
      parameter = new io.dekorate.halkyon.model.Parameter();
