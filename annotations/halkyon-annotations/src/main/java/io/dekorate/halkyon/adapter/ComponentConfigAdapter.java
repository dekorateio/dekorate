...
public class ComponentConfigAdapter {
    ...
    return new ComponentConfigBuilder(new io.dekorate.halkyon.config.ComponentConfig(null,
      ...
      Arrays.asList(instance.provides()).stream().map(i -> new io.dekorate.halkyon.config.CapabilityConfig(null,
        ...
        Arrays.asList(i.parameters()).stream().map(j -> new io.dekorate.halkyon.config.Parameter(j.name(),
          j.value())).collect(Collectors.toList()).toArray(new io.dekorate.halkyon.config.Parameter[0]))).collect(Collectors.toList()).toArray(new io.dekorate.halkyon.config.CapabilityConfig[0]),
      Arrays.asList(instance.requires()).stream().map(i -> new io.dekorate.halkyon.config.RequiredCapabilityConfig(null,
        null,
        i.name(),
        i.category(),
        i.type(),
        ...
        i.autoBindable(),
        Arrays.asList(i.parameters()).stream().map(j -> new io.dekorate.halkyon.config.Parameter(j.name(),
          j.value())).collect(Collectors.toList()).toArray(new io.dekorate.halkyon.config.Parameter[0]))).collect(Collectors.toList()).toArray(new io.dekorate.halkyon.config.RequiredCapabilityConfig[0])));
  }
  ...
  private static ComponentConfig getComponentConfig(Map map) {
    return new ComponentConfig(
      ...
      Arrays.stream((Map[]) map.getOrDefault("provides", new Map[0])).map(ComponentConfigAdapter::getProvidedCapability).toArray(CapabilityConfig[]::new),
      Arrays.stream((Map[]) map.getOrDefault("requires", new Map[0])).map(ComponentConfigAdapter::getRequiredCapability).toArray(RequiredCapabilityConfig[]::new));
  }
