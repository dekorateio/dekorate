...
public class KnativeExtension implements ExecutionCondition, BeforeAllCallback, AfterAllCallback, TestWatcher,
    WithKnativeIntegrationTestConfig, WithPod, WithKnativeRoute, WithKubernetesClient, WithKnativeClient, WithKnativeResources,
    WithKnativeService, WithEvents, WithProject, WithKnativeConfig, WithImageConfig {

  private static final String SERVICE_CONDITION_READY = "Ready";
  ...
  public ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext context) {
    try {
      ...
      String reason = "Could not communicate with Knative API server.";
      LOGGER.error(reason);
      return ConditionEvaluationResult.disabled(reason);
    }
  }
  ...
  public void afterAll(ExtensionContext context) {
    KnativeIntegrationTestConfig config = getKnativeIntegrationTestConfig(context);
    ...
    try {
      ...
      closeKnativeClient(context);
    }
  }

  @Override
  public String[] getAdditionalModules(ExtensionContext context) {
    return getKnativeIntegrationTestConfig(context).getAdditionalModules();
  }
  ...
  private void startProject(ExtensionContext context, Project project) throws InterruptedException {
    ...
    if (config.isDeployEnabled()) {
      ...
      waitUntilCondition(context, waitables,
          i -> {
            if (i instanceof Service) {
              return isServiceReady((Service) i);
            } else {
              return Readiness.getInstance().isReady(i);
            }
          },
          config.getReadinessTimeout(),
  ...
  private static boolean isServiceReady(Service service) {
    Utils.checkNotNull(service, "Service can't be null.");
    if (service.getStatus() != null && service.getStatus().getConditions() != null) {
      return service.getStatus().getConditions().stream()
          .anyMatch(condition -> SERVICE_CONDITION_READY.equals(condition.getType())
              && Boolean.TRUE.toString().equalsIgnoreCase(condition.getStatus()));
    }
